...
void UART_InitWithInt(unsigned int uiBaudRate){
   ...
   // UART0
   PINSEL0 |= (1<<2);                  													// ustawic pina na odbiornik uart0
   U0LCR  |= m8BIT_UART_WORD_LENGTH | mDIVISOR_LATCH_ACCES_BIT; // długosc słowa, DLAB = 1
   U0DLL   = (15000000/16)/uiBaudRate;                      		// predkosc transmisji
   U0LCR  &= (~mDIVISOR_LATCH_ACCES_BIT);                       // DLAB = 0
   U0IER  |= mRX_DATA_AVALIABLE_INTERRUPT_ENABLE;               // zalacza przerwanie Receive Data Available dla UART0 (irq gdy w buforze Rx pojawi sie bajt
